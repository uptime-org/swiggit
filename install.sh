...
EOF
echo ""
echo "🔍 Config Options Explained:"
echo "  git_user_name       - Your git commit author name"
echo "  git_user_email      - Your git commit author email"
echo "  ssh_key_path        - Path to SSH private key for GitHub auth"
echo "  github_username     - Your GitHub username/account name"
echo "  github_token        - Personal access token for API calls"
echo "  expected_user       - Expected GitHub user (for validation)"
echo ""
echo "🔑 GitHub Token:"
...
echo ""
echo "🔗 SSH Key Setup:"
echo "  - Use existing SSH key: ~/.ssh/id_rsa (most common)"
echo "  - Or project-specific key: ./.ssh/id_rsa"
echo "  - Make sure key is added to your GitHub account"
echo "  - Test with: ssh -T git@github.com"
echo ""
echo "⚙️  Advanced Options (via swiggit init):"
echo "  - Multiple organizations support"
echo "  - Project-specific settings (name, description)"
echo "  - CLI behavior options (auto_push, verbose_output)"
echo "  - SSH connection options"
echo "  - Default repo settings (license, visibility)"
echo ""
echo "📚 Commands:"
